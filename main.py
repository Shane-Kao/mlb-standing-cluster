...
data = get_standing()
cluster_data = [[float(i['sport_games_back'])] if i['sport_games_back'] != '-'
...
schedule = [dict(score=rank_dict[i['away_team']] + rank_dict[i['home_team']],
                 **i) for i in schedule]

for i in sorted(schedule, key=lambda x: x['score']):
    print(i["away_team"], 'vs', i["home_team"], i["score"], i["media"])
    print()